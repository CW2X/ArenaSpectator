...
local PET_ICONS = "Interface\\Icons\\"
...
local SIZE = {
    ...
    ["BIG"] = { -- Big frames (current POV, POV's target)
        ...
        ["NAMETEXTSIZE"] = 14, -- Player name font size
        ["HEALTHHEIGHT"] = 24, -- Healthbar height
        ["HEALTHTEXTSIZE"] = 17, -- Health text size
        ["POWERTEXTSIZE"] = 9, -- Power bar text size
        ...
        ["FRAMEFROMBORDER"] = 120, -- Frame spacing from bottom of screen
        ["SPELLSIZE"] = 40 -- Spell display size
    },
...
local PLAYERPETS = {
    ...
    [35] = "ability_druid_naturalperfection",
...
-- bool value to hide frame
local hideframes = false
...
local function IsNotArenaPlayerByGUID(guid)
	for _, p in pairs(ATPlayers) do
		if p.guid == guid then
			return false
		end
...
local function CreateFrameForPlayer(p)
	if IsNotArenaPlayerByGUID(p.guid)  then
		return
	end
    ...
    fpet1:SetTexture(PET_ICONS .. PLAYERPETS[0])
...
-- SendChatMessage(value, "CHANNEL", nil, 5)
...
local function UpdateSpell(target, id, casttime)
    ...
    if casttime > 0 or casttime < 0 then
        for _, barname in pairs(ALLBARS) do
			...
			ATPlayers[target][barname].cast.icon:SetHeight(24)
			ATPlayers[target][barname].cast.icon:SetWidth(24)
...
local function UpdatePetTexture(target, value)
    ATPlayers[target].fsmall.pettext:SetTexture(PET_ICONS .. PLAYERPETS[value])
end
...
local function ACPEC_ShowOutdatedAddonVersion()
    StaticPopupDialogs["ARENASPECTATOR_OUTDATED"] = {
        text = "Your version of Arena-Spectator UI is outdated. Please visit http://wowcircle.com/ to download last version.",
        button1 = "Ok",
        whileDead = true,
        timeout = 0,
        hideOnEscape = false,
...
local function LeaveArenaUI()
    LeaveBattlefield()
end

local function ResetViewUI()
...
local function HideFrameUI()
	if (hideframes == false) then
		hideframes = true
		for _, p in pairs(ATPlayers) do
			p.fsmall.main:Hide()
            p.fself.main:Hide()
...
local function init()
    ...
    leavearena:SetHeight(15)
    leavearena:SetWidth(120)
    leavearena:SetPoint("TOPLEFT", 260, 0)
    leavearena.texture = leavearena:CreateTexture()
    leavearena.texture:SetAllPoints(leavearena)
    leavearena.texture:SetTexture(0.6, 0.6, 0.2)
    leavearena:SetScript("OnClick", LeaveArenaUI)
    ...
    resetview:SetHeight(15)
    resetview:SetWidth(120)
    resetview:SetPoint("TOPLEFT", 390, 0)
    resetview.texture = resetview:CreateTexture()
    resetview.texture:SetAllPoints(resetview)
    resetview.texture:SetTexture(0.6, 0.6, 0.2)
    resetview:SetScript("OnClick", ResetViewUI)
	...
	-- Create hide frame button
    hideframe = CreateFrame("Button", nil, WorldFrame)
    hideframe:SetHeight(15)
    hideframe:SetWidth(120)
    hideframe:SetPoint("TOPLEFT", 700, 0)
    hideframe.texture = hideframe:CreateTexture()
    hideframe.texture:SetAllPoints(hideframe)
    hideframe.texture:SetTexture(0.6, 0.6, 0.2)
    hideframe:SetScript("OnClick", HideFrameUI)
    hideframe.text = hideframe:CreateFontString()
    hideframe.text:SetFont(STANDARD_TEXT_FONT, 12, "OUTLINE")
    hideframe.text:SetPoint("CENTER", 0, 0)
	if GetLocale() == "ruRU" then
		hideframe.text:SetText(TEXT.HIDEFRAMERU)
	else
		hideframe.text:SetText(TEXT.HIDEFRAME)
	end
	
    -- Create fix interface button
    fixinterface = CreateFrame("Button", nil, WorldFrame)
    fixinterface:SetHeight(15)
    fixinterface:SetWidth(120)
    fixinterface:SetPoint("TOPLEFT", 830, 0)
    fixinterface.texture = fixinterface:CreateTexture()
    fixinterface.texture:SetAllPoints(fixinterface)
    fixinterface.texture:SetTexture(0.6, 0.6, 0.2)
    fixinterface:SetScript("OnClick", FixFrames)
